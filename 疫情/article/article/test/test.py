        ...
        for new in news:
            ...
            if len(new_text) >= 10:
                ...
                if new_text.startswith('截至'):
                    ...
                    sh_dict['累计重症'] = int(re.search(style2, new_text).group(1)) + int(
                        re.search(style2, new_text).group(2))
...
def get_all_today_data(url):
    ...
    try:
        ...
        # print(news)
        style1 = re.compile(
            '新增确诊病例(\d+)例.*?（湖北省(\d+)例.*?重症病例(\d+)例.*?湖北省(\d+)例.*?死亡病例(\d+)例.*?湖北省(\d+)例.*?治愈出院病例(\d+)例.*?湖北省(\d+)例.*?疑似病例(\d+)例.*?湖北省(\d+)例')
